...
typedef struct {
    u32 addr;
    u32 size;
} XPyr_dense_optical_flow_accel_uio_map;
...
typedef struct {
    ...
    XPyr_dense_optical_flow_accel_uio_map maps[ MAX_UIO_MAPS ];
} XPyr_dense_optical_flow_accel_uio_info;

/***************** Variable Definitions **************************************/
static XPyr_dense_optical_flow_accel_uio_info uio_info;
...
static int uio_info_read_name(XPyr_dense_optical_flow_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPyr_dense_optical_flow_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPyr_dense_optical_flow_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPyr_dense_optical_flow_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPyr_dense_optical_flow_accel_Initialize(XPyr_dense_optical_flow_accel *InstancePtr, const char* InstanceName) {
	XPyr_dense_optical_flow_accel_uio_info *InfoPtr = &uio_info;
